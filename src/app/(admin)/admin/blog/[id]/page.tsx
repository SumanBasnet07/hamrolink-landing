"use client";
// app/admin/blog/[id]/page.tsx — edit post + moderate comments

import { useState, useEffect } from "react";
import { useRouter, useParams } from "next/navigation";
import Link from "next/link";
import {
  Loader2, ArrowLeft, Check, X, Trash2,
...
export default function EditPostPage() {
  ...
  if (error) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4">
      ...
      <Link href="/admin/blog" className="text-indigo-600 text-sm font-bold hover:underline">← Back to posts</Link>
    </div>
  );
      ...
      {tab === "comments" && (
        <div className="max-w-3xl mx-auto px-6 py-8 space-y-8">
          ...
          <div>
            ...
            {pending.length === 0 ? (
              <p className="text-gray-400 text-sm bg-white rounded-2xl p-6 border border-gray-200 text-center">
                No pending comments. 🎉
              </p>
            ) : (
                  ...
                  <div key={c._id} className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          onClick={() => moderate(c._id, "approve_comment")}
                          disabled={moderating === c._id}
                          className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-500 transition-colors disabled:opacity-50">
                          {moderating === c._id ? <Loader2 className="w-3 h-3 animate-spin"/> : <Check className="w-3 h-3"/>}
                          Approve
                        </button>
                        <button
                          onClick={() => moderate(c._id, "delete_comment")}
                          disabled={moderating === c._id}
