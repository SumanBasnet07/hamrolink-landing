...
export default function PricingPage({ params }: { params: any }) {
  ...
  const gridPlans = PLANS.filter((plan) => plan.name !== "local_start").sort((a, b) => {
    // Exact order: free, business, institution
    const order: Record<string, number> = { free: 0, business: 1, institution: 2 };
    return (order[a.name] ?? 99) - (order[b.name] ?? 99);
  });
  const localStarter = PLANS.find((plan) => plan.name === "local_start");
